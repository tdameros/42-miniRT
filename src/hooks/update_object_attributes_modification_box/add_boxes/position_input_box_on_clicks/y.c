...
/*   Updated: 2023/07/30 18:32:23 by vfries           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			...
			t_click_data click_data)
{
	...
	else if (object != NULL)
	{
		...
		if (recalculate_bvh_scene(&engine->scene, object) < 0)
			ft_fatal_error("position_input_box_y_on_click_plus: recalculate_bvh_scene failed");
	}
}
			...
			t_click_data click_data)
{
	...
	else if (object != NULL)
	{
		...
		update_xyz_float_input_boxes(engine, object->position,
			&engine->gui.float_input_boxes.position);
		if (recalculate_bvh_scene(&engine->scene, object) < 0)
			ft_fatal_error("position_input_box_y_on_click_minus: recalculate_bvh_scene failed");
	}
}
