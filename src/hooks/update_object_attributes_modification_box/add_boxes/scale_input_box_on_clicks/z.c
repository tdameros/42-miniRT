			...
			t_click_data click_data)
{
	...
	if (recalculate_bvh_scene(&engine->scene, object) < 0)
		ft_fatal_error("scale_input_box_z_on_click_plus: recalculate_bvh_scene failed");
}
			...
			t_click_data click_data)
{
	...
	update_xyz_float_input_boxes(engine, object->cache.mesh.scale_vector,
		&engine->gui.float_input_boxes.scale);
	if (recalculate_bvh_scene(&engine->scene, object) < 0)
		ft_fatal_error("scale_input_box_z_on_click_minus: recalculate_bvh_scene failed");
}
