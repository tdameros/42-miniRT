...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/09/24 01:12:55 by tdameros          #+#    #+#             */
/*   Updated: 2023/09/24 01:12:55 by tdameros         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void	add_delete_box(t_engine *engine, t_gui_box *gui_box, t_gui_box *parent)
{
	...
	if (engine->gui.selected_object.object != NULL)
		text = DELETE_OBJECT_BUTTON_TEXT;
	else if (engine->gui.selected_object.light != NULL)
		text = DELETE_LIGHT_BUTTON_TEXT;
	else
		text = DELETE_BUTTON_ERROR_TEXT;
	write_centered_string_to_image(&engine->gui.font, &gui_box->image, text);
	write_centered_string_to_image(&engine->gui.font,
		&gui_box->on_hover_image, text);
