...
/*   Updated: 2023/09/24 09:05:32 by vfries           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			...
			t_engine *engine, t_click_data click_data)
{
	...
	if (object == NULL)
		return ;
	if ((int)object->material.textures.outline.checkerboard.size.y % 2)
		object->material.textures.outline.checkerboard.size.y++;
	object->material.textures.outline.checkerboard.size.y
		= (int)object->material.textures.outline.checkerboard.size.y + 2;
			...
			t_engine *engine, t_click_data click_data)
{
	...
	if (click_data.button != BUTTON_LEFT)
		return (outline_checkerboard_size_input_box_y_on_click_text(self,
				engine, click_data));
	object = engine->gui.selected_object.object;
	if (object == NULL || object->material.textures.outline.checkerboard.size.y
		<= 1.f)
		return ;
	if ((int)object->material.textures.outline.checkerboard.size.y % 2)
		object->material.textures.outline.checkerboard.size.y++;
	...
	if (object->material.textures.outline.checkerboard.size.x <= 1.f
		...
		disable_outline_checkerboard(engine, object);
	engine->scene_changed = true;
	update_xy_float_input_boxes(engine,
		vector2f_divide(object->material.textures.outline.checkerboard.size, \
		2.f), &engine->gui.float_input_boxes.outline_checkerboard_size);
